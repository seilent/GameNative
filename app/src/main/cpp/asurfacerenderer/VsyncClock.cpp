...
#define VLOG(...) __android_log_print(ANDROID_LOG_INFO, "asr_vsync", __VA_ARGS__)
...
void VsyncClock::threadMain() {
    ...
    if (!lp) {
        VLOG("ALooper_prepare failed; host vsync pacing disabled");
        running.store(false);
        return;
    }
    looper.store(lp);
    ...
    if (!fnGetInstance || !fnPostFrameCallback64) {
        VLOG("AChoreographer symbols missing; host vsync pacing disabled");
        looper.store(nullptr);
        running.store(false);
        return;
    }
    ...
    if (!choreographer) {
        VLOG("AChoreographer_getInstance returned null; host vsync pacing disabled");
        looper.store(nullptr);
        running.store(false);
        return;
    }
